...
def run_demo():
    ...
    rule1 = memory_system.create(
        content="Apache docs must be 100% English", tags=["rule", "documentation"], source="user"
    )
    ...
    for i, rule in enumerate(critical_rules, 1):
        ...
        print(
            f"     Priority: {rule.priority}/5, Confidence: {rule.confidence:.0%}, Source: {rule.source}"
        )
    ...
    if result.needs_confirmation:
        print(
            f"  Needs confirmation: {len(result.low_confidence_memories)} low-confidence memories"
        )
    print()
    ...
    test_memory = memory_system.create(content="Test preference", tags=["test"], source="inferred")
    ...
    for memory in memory_system.all():
        ...
        print(
            f"    Confidence: {memory.confidence:.0%}, Source: {memory.source}, Priority: {memory.priority}/5"
        )
    print()
    ...
    print(
        f"  ✅ Pre-action check: {3 if not checker.check('Write Chinese documentation').allowed else 0}/3 violations detected"
    )
