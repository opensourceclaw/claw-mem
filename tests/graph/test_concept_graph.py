...
class TestConceptMediatedGraph:
    """Test concept-mediated graph"""

    def setup_method(self):
        self.graph = ConceptMediatedGraph(embedder=DummyEmbedder())

    def test_add_single_episode(self):
        node_id = self.graph.add_episode(content="User says: Hello", speaker="user")
        assert node_id is not None
        node = self.graph.get_node(node_id)
        assert node is not None
        assert node.content == "User says: Hello"

    def test_add_conversation(self):
        turns = [
            {"speaker": "user", "content": "I want to learn Python"},
            {"speaker": "agent", "content": "Start with basic syntax"},
        ]
    ...
    def test_add_fact(self):
        ...
        fact_id = self.graph.add_fact(
            content="Python is a programming language", source_episode_id=ep_id, confidence=0.9
        )
    ...
    def test_add_reflection(self):
        ...
        ref_id = self.graph.add_reflection(
            content="User is interested in programming",
...
class TestRetrieve:
    ...
    def test_retrieve_with_results(self):
        self.graph.add_episode("Python is a programming language")
        self.graph.add_episode("JavaScript is used for web development")
    ...
    def test_retrieve_with_alpha(self):
        self.graph.add_episode("Python programming")
        self.graph.add_episode("JavaScript development")
    ...
    def test_retrieve_limit_k(self):
        for i in range(20):
            self.graph.add_episode(f"Content {i}")

        results = self.graph.retrieve("Content", k=5)
