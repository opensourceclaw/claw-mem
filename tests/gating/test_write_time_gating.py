...
class TestWriteTimeGating:
    ...
    def test_high_salience_stores_to_active(self):
        ...
        result = gating.write(
            {"content": "重要决策", "source": "user", "context": {"session": "test"}}
        )
    ...
    def test_low_salience_stores_to_cold(self):
        ...
        # Use low salience content
        result = gating.write(
            {
                "content": "hello world hello world",  # Repeated content has low novelty
                "source": "external",
                "context": {},
            }
        )
        ...
        assert result.stored is True
        # Result may be active or cold, depending on scoring
    ...
    def test_threshold_adjustment(self):
        ...
        # Higher threshold more likely stores to cold
    ...
    def test_should_store(self):
        ...
        high_salience = gating.should_store(
            {"content": "重要决策", "source": "user", "context": {}}
        )

        low_salience = gating.should_store(
            {"content": "hello", "source": "external", "context": {}}
        )

        assert high_salience is True
        # low_salience depends on scoring
...
class TestSalienceScorer:
    ...
    def test_source_reputation_user(self):
        ...
        score = scorer.compute({"content": "test", "source": "user", "context": {}})

        # User source should have higher score
        assert score >= 0.4

    def test_source_reputation_agent(self):
        """Test agent source"""
        scorer = SalienceScorer()
        score = scorer.compute({"content": "test", "source": "agent", "context": {}})
    ...
    def test_novelty_first_item(self):
        ...
        # First item should have highest novelty
        score = scorer.compute({"content": "unique content here", "source": "user", "context": {}})
    ...
    def test_reliability_with_context(self):
        ...
        with_context = scorer.compute(
            {"content": "test", "source": "user", "context": {"session": "test", "timestamp": 123}}
        )

        without_context = scorer.compute({"content": "test", "source": "user", "context": {}})

        # With context should have higher or equal score
...
class TestPerformance:
    ...
    def test_write_latency(self):
        ...
        start = time.time()
        for i in range(100):
            gating.write({"content": f"test content {i}", "source": "user", "context": {}})
    ...
    def test_salience_computation_latency(self):
        ...
        start = time.time()
        for i in range(100):
            scorer.compute({"content": f"test content {i}", "source": "user", "context": {}})
        elapsed = (time.time() - start) * 1000
