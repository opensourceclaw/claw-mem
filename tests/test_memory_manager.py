...
class TestMemoryManager:
    ...
    def test_store_with_metadata(self, temp_workspace):
        ...
        assert len(results) > 0
        found = any(
            r.get("metadata") == {"neo_agent": "Tech", "neo_domain": "Work"} for r in results
        )
    ...
    def test_search_with_metadata_filter(self, temp_workspace):
        ...
        memory.store(
            "Life memory",
            memory_type="semantic",
            metadata={"neo_agent": "Body", "neo_domain": "Life"},
        )
        ...
        results = memory.search(
            "memory", metadata={"neo_agent": "Body", "neo_domain": "Life"}, mode="keyword"
        )
