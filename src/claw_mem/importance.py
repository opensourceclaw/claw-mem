...
class MemoryImportance:
    ...
    def calculate_total(self) -> float:
        ...
        self.total_score = min(
            2.0, self.base_score + self.type_weight + self.frequency_weight + self.recency_weight
        )
...
class ImportanceScorer:
    ...
    def rank_memories(self, memories: list) -> list:
        ...
        for memory in memories:
            ...
            scored_memories.append(
                {"memory": memory, "importance": importance, "score": importance.total_score}
            )
    ...
    def filter_high_priority(
        self, memories: list, threshold: float = 1.5, limit: Optional[int] = None
    ) -> list:
        """
    ...
    def explain_score(self, memory: dict) -> str:
        ...
        if accessed_at:
            ...
            explanation += (
                f"  - Recency weight: {importance.recency_weight:.1f} ({days} days ago)\n"
            )
