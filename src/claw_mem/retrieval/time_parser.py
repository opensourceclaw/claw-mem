...
class TimeExpressionParser:
    ...
    def __init__(self):
        ...
        self.patterns = [
            ...
            (
                r"\b(yesterday|today|last\s+week|last\s+month|last\s+year)\b",
    ...
    def extract_time_value(self, text: str) -> Optional[str]:
        ...
        match = re.search(
            r"\b(yesterday|today|last\s+week|last\s+month|last\s+year)\b", text, re.IGNORECASE
        )
        if match:
            return match.group(1)
    ...
    def filter_memories_by_time(
        self, memories: List[Dict], start_time: datetime, end_time: datetime
    ) -> List[Dict]:
        """
