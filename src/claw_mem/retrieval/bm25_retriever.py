    ...
    def explain(self, query: str, memory_idx: int) -> Dict:
        ...
        for token in query_tokens:
            if token in doc_tokens:
                ...
                token_scores[token] = {"tf": tf, "idf": idf, "score": score}
...
class HybridBM25Retriever:
    ...
    def __init__(
        ...
        self.bm25_retriever = BM25Retriever(
            k1=k1, b=b, recency_boost=recency_boost, frequency_boost=frequency_boost
        )
    ...
    def search(self, query: str, memories: List[Dict], limit: int = 10) -> List[Dict]:
        ...
        for idx, result in enumerate(bm25_results):
            ...
            result_scores[memory_id] = (
                result_scores.get(memory_id, 0) + self.bm25_weight * bm25_score
            )

        for idx, result in enumerate(keyword_results):
            memory_id = result.get("id", idx)
            keyword_score = (
                (len(keyword_results) - idx) / len(keyword_results) if keyword_results else 0
            )
            result_scores[memory_id] = (
