...
class TTLCache:
    ...
    def cleanup_expired(self) -> int:
        ...
        """
        now = time.time()
        expired_keys = [key for key, ts in self.timestamps.items() if now - ts >= self.ttl_seconds]
...
class OptimizedRetriever:
    ...
    def _generate_cache_key(
        self, query: str, memory_type: Optional[str], limit: int, rank_by_importance: bool
    ) -> str:
        """
    ...
    def search(
        ...
        results = self._search_without_cache(
            query, episodic, semantic, procedural, memory_type, limit, rank_by_importance
        )
