...
class EntityRecognizer:
    ...
    def _extract_entities_regex(self, text: str) -> List[Entity]:
        ...
        for match in re.finditer(person_pattern, text):
            entities.append(
                Entity(text=match.group(), label="PERSON", start=match.start(), end=match.end())
            )
        ...
        for match in re.finditer(org_pattern, text):
            entities.append(
                Entity(text=match.group(), label="ORG", start=match.start(), end=match.end())
            )
        ...
        for match in re.finditer(gpe_pattern, text):
            ...
            if not any(e.text == text_snippet for e in entities):
                entities.append(
                    Entity(text=text_snippet, label="GPE", start=match.start(), end=match.end())
                )
...
class EntityEnhancedRetriever:
    ...
    def __init__(
        self, use_spacy: bool = True, entity_weight: float = 0.3, bm25_weight: float = 0.7
    ):
        """
    ...
    def calculate_entity_score(self, query_entities: Set[str], memory_entities: Set[str]) -> float:
        """
    ...
    def search(self, query: str, memories: List[Dict], limit: int = 10) -> List[Dict]:
        ...
        for memory in bm25_results:
            ...
            # Combine scores
            combined_score = self.bm25_weight * bm25_score + self.entity_weight * entity_score
    ...
    def search(self, query: str, memories: List[Dict], limit: int = 10) -> List[Dict]:
        ...
        for idx, memory in enumerate(bm25_results):
            ...
            result_scores[memory_id] = (
                result_scores.get(memory_id, 0) + self.bm25_weight * normalized_bm25
            )
        ...
        for idx, memory in enumerate(bm25_results):
            ...
            if query_entities and memory_entities:
                ...
                result_scores[memory_id] = (
                    result_scores.get(memory_id, 0) + self.entity_weight * entity_score
                )
        ...
        for idx, memory in enumerate(keyword_matches):
            memory_id = memory.get("id", idx)
            keyword_score = (
                (len(keyword_matches) - idx) / len(keyword_matches) if keyword_matches else 0
            )
            result_scores[memory_id] = (
                result_scores.get(memory_id, 0) + self.keyword_weight * keyword_score
            )
