...
class Timeline:
    ...
    def query(self, query: TimelineQuery) -> List[TimelineEvent]:
        ...
        if query.start_date and query.end_date:
            ...
            # Search all years
            years = sorted(
                set(int(f.stem.split("_")[1]) for f in self.timeline_dir.glob("year_*.json"))
            )
    ...
    def _matches_query(self, event: TimelineEvent, query: TimelineQuery) -> bool:
        ...
        if query.text_search:
            ...
            if (
                search_lower not in event.title.lower()
                and search_lower not in event.description.lower()
            ):
                return False
