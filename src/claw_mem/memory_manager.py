...
class MemoryManager:
    """Three-layer memory architecture: store, search, context injection, with optional gating/decay/graph/CMS."""
    ...
    def query_cache(self) -> Optional[QueryCache]:
        ...
        if self._query_cache is None and self.enable_cache:
            self._query_cache = self._factory.create_query_cache()
    ...
    def synonym_expander(self) -> Optional[SynonymExpander]:
        ...
        if self._synonym_expander is None and self.enable_synonyms:
            self._synonym_expander = self._factory.create_synonym_expander()
    ...
    def search_stats(self) -> Optional[SearchStats]:
        ...
        if self._search_stats is None and self.enable_stats:
            self._search_stats = self._factory.create_search_stats()
    ...
    def get_search_statistics(self) -> Optional[Dict]:
        """Get search performance statistics (latency, cache hits, accuracy)."""
    ...
    def batch_search(
        ...
        """Run search() for each query, return list of result lists."""
        return [
    ...
    def reflect(self, user_id: str = "", force: bool = False) -> ReflectionResult:
        """Execute a reflection cycle: collect memories, extract observations, synthesize beliefs."""
    ...
    def get_beliefs(self, user_id: str = "", include_history: bool = False) -> List[Dict]:
        """Get current beliefs, optionally with version history."""
    ...
    def search(
        ...
        Args:
            ...
            mode: Search mode - "keyword" | "three_tier" (default: use self.search_mode)
        ...
        if self.synonym_expander:
            expanded_query = self.synonym_expander.expand(query)
        ...
        # Perform search (keyword by default; three_tier uses separate path)
        if search_mode == "three_tier":
            session_context = {
                "working_memory": self.working_memory,
                "session_id": self.session_id,
            }
            results_raw = self.three_tier_retriever.search(
            ...
            # keyword mode (default) — uses original query for substring matching
