...
class MemoryTags:
    ...
    def to_dict(self) -> Dict:
        return {"memory_id": self.memory_id, "tags": list(self.tags), "updated_at": self.updated_at}
...
class MemoryLinkParser:
    ...
    def parse_links(self, content: str, source_id: str) -> List[MemoryLink]:
        ...
        for match in self.LINK_PATTERN.finditer(content):
            ...
            links.append(
                MemoryLink(source_id=source_id, target_id=target_id, link_type="reference")
            )
    ...
    def extract_link_targets(self, content: str) -> List[str]:
        ...
        """
        return [match.group(1).strip() for match in self.LINK_PATTERN.finditer(content)]
...
class MemoryLinkManager:
    ...
    def process_memory(self, memory_id: str, content: str) -> Tuple[List[MemoryLink], Set[str]]:
        """
    ...
    def _update_tags(self, memory_id: str, tags: Set[str]) -> None:
        ...
        self.tags[memory_id] = MemoryTags(
            memory_id=memory_id, tags=tags, updated_at=datetime.now().isoformat()
        )
    ...
    def export_links(self) -> Dict:
        ...
        return {
            "links": {
                source: [link.to_dict() for link in links] for source, links in self.links.items()
            },
            "tags": {memory_id: tags.to_dict() for memory_id, tags in self.tags.items()},
            "tag_index": {tag: list(memories) for tag, memories in self.tag_index.items()},
        }
