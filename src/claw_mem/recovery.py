...
class RecoveryManager:
    ...
    def diagnose(self, error: Exception) -> Diagnosis:
        ...
        if "index" in error_msg.lower() or "Index" in error_type:
            ...
            root_cause = (
                "May be caused by unexpected shutdown, disk errors, or version incompatibility"
            )
    ...
    def recover(self, error: Exception, context: Dict[str, Any] = None) -> RecoveryResult:
        ...
        try:
            ...
            if result.success:
                ...
                print(
                    f"✅ Recovery successful in {elapsed:.2f}ms using {result.strategy_used.value}"
                )
            else:
            ...
            if total > 0:
                self.stats["avg_recovery_time_ms"] = (
                    self.stats["avg_recovery_time_ms"] * (total - 1) + elapsed
                ) / total
    ...
    def _recover_index(
        self, error: Exception, context: Dict, diagnosis: Diagnosis
    ) -> RecoveryResult:
