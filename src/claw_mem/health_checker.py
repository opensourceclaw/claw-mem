...
class HealthChecker:
    ...
    def _check_disk_space(self) -> HealthStatus:
        ...
        try:
            ...
            if free_percent < 5:
                severity = "critical"
                message = (
                    f"Critical: Only {free_percent:.1f}% disk space remaining ({free_gb:.2f}GB)"
                )
    ...
    def _check_expired_memories(self) -> HealthStatus:
        ...
        try:
            ...
            if not memory_dir.exists():
                return HealthStatus(
                    component="memories", healthy=True, message="No memories yet", severity="info"
                )
    ...
    def _check_backup_status(self) -> HealthStatus:
        ...
        try:
            backup_dir = Path(
                self.config.get("storage.backup_dir", "~/.claw-mem/backups")
            ).expanduser()

            if not backup_dir.exists():
                return HealthStatus(
                    component="backups", healthy=True, message="No backups yet", severity="info"
                )
    ...
    def _cleanup_old_backups(self) -> int:
        ...
        try:
            ...
            backup_files = sorted(
                backup_dir.glob("*.zip"), key=lambda f: f.stat().st_mtime, reverse=True
            )
