...
class MemoryFixPlugin:
    ...
    def store_with_dedup(
        self, content: str, memory_type: str = "semantic", tags: Optional[List[str]] = None
    ) -> Tuple[bool, str]:
        """
    ...
    def _find_similar_memory(self, content: str, memories: List[Dict]) -> Optional[Dict]:
        ...
        for i, memory in enumerate(memories):
            ...
            if similarity > 0.7:  # 70% similarity threshold
                return {"index": i, "memory": memory, "similarity": similarity}
        return None
    ...
    def _add_new_memory(
        self, content: str, memory_type: str, tags: Optional[List[str]] = None
    ) -> Tuple[bool, str]:
        """Add new memory"""
        try:
    ...
    def _update_memory(
        self, index: int, content: str, tags: Optional[List[str]] = None
    ) -> Tuple[bool, str]:
    ...
    def _write_memories(self, memories: List[Dict]):
        ...
        with open(self.memory_file, "w", encoding="utf-8") as f:
            ...
            for memory in memories:
                tags_str = (
                    f" <!-- tags: {', '.join(memory['tags'])} -->" if memory.get("tags") else ""
                )
...
if __name__ == "__main__":
    ...
    success, msg = plugin.store_with_dedup(
        "User prefers Chinese language", memory_type="semantic", tags=["preference", "language"]
    )
    print(f"  {msg}")
