...
class FriendlyError(Exception):
    ...
    def __init__(
        ...
        if isinstance(message, str) and all(
            arg is None or isinstance(arg, str) for arg in [suggestion, error_code, details]
        ):
...
class IndexNotFoundError(FriendlyError):
    ...
    def __init__(self, index_path: str, suggestion: Optional[str] = None):
        super().__init__(
            ...
            or "First startup requires index rebuild, please wait (~1 second)",
...
class WorkspaceNotFoundError(FriendlyError):
    ...
    def __init__(self, searched_paths: list, suggestion: Optional[str] = None):
        ...
        super().__init__(
            ...
            suggestion=suggestion
            or "Please confirm OpenClaw is installed or manually specify workspace path",
...
class MemoryCorruptedError(FriendlyError):
    ...
    def __init__(self, file_path: str, suggestion: Optional[str] = None):
        super().__init__(
            ...
            suggestion=suggestion
            or "System will auto-recover from recent backup. If issue persists, check disk health",
...
class PermissionDeniedError(FriendlyError):
    ...
    def __init__(self, path: str, suggestion: Optional[str] = None):
        super().__init__(
            ...
            suggestion=suggestion
            or "Please check file permissions or use chmod to modify permissions",
...
class ConfigurationError(FriendlyError):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(args) == 1 and isinstance(args[0], str) and not kwargs:
            ...
            super().__init__(message=args[0], error_code="CONFIGURATION_ERROR")
        else:
            ...
            super().__init__(
                ...
                suggestion=suggestion
                or "Please check configuration file or run 'claw-mem --help' for available options",
...
class MemoryRetrievalError(FriendlyError):
    ...
    def __init__(self, query: str, suggestion: Optional[str] = None):
        super().__init__(
            ...
            suggestion=suggestion
            or "Try simplifying search keywords or check if memory file exists",
...
class ValidationError(FriendlyError):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(args) == 1 and isinstance(args[0], str) and not kwargs:
            ...
            super().__init__(message=args[0], error_code="VALIDATION_ERROR")
        else:
...
if __name__ == "__main__":
    ...
    try:
        raise WorkspaceNotFoundError(
            ["~/.openclaw/workspace", "~/.config/openclaw/workspace", "/current/dir"]
        )
    except FriendlyError as e:
        print(e)
        print()
