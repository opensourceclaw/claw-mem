...
class InMemoryIndex:
    ...
    def __init__(
        self, ngram_size: int = 3, index_dir: Optional[str] = None, enable_persistence: bool = True
    ):
        """
        ...
        self.index_dir = (
            Path(index_dir).expanduser() if index_dir else Path.home() / ".claw-mem" / "index"
        )
    ...
    def save_index(self) -> bool:
        ...
        try:
            ...
            print(
                f"💾 Index saved: {len(self.memory_ids)} memories, {len(self.ngram_index)} n-grams ({file_size_kb:.1f} KB)"
            )
            return True
    ...
    def bm25_search(self, query: str, limit: int = 10) -> List[Tuple[str, float]]:
        ...
        # Return top results
        results = [(self.memory_ids[i], score) for i, score in ranked[:limit] if score > 0]

        return results

    def hybrid_search(
        self, query: str, limit: int = 10, ngram_weight: float = 0.7, bm25_weight: float = 0.3
    ...
    def _remove_stopwords(self, tokens: List[str], chinese: bool = False) -> List[str]:
        ...
        if chinese:
            stopwords = {
                # Chinese stop words for text processing
                "的",
                "了",
                "是",
                "在",
    ...
    def verify_integrity(self) -> Tuple[bool, List[str]]:
        ...
        if len(self.memory_ids) != len(self.documents):
            issues.append(
                f"Memory/document count mismatch: {len(self.memory_ids)} vs {len(self.documents)}"
            )
...
class WorkingMemoryCache:
    ...
    def put(self, memory_id: str, data: Dict) -> None:
        ...
        # Add to cache
        self.cache[memory_id] = {"data": data, "timestamp": datetime.now().timestamp()}
