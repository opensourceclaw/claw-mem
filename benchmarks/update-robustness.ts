...
export class UpdateRobustnessBenchmark extends BenchmarkCore {
  private latencies: number[] = [];
  private originalPrefs: FactRecord[] = [];
  private updatedPrefs: FactRecord[] = [];
  ...
  protected async runQueries(data: BenchmarkData): Promise<BenchmarkDetail[]> {
    ...
    for (const query of data.queries) {
      ...
      if (query.queryType === "latest") {
        ...
        actualContent = typedResult.map(r => (r.content as string) || "").join(" | ");
      }
  ...
  private generateUpdateQueries(): QueryRecord[] {
  ...
  private scoreUpdateQuery(query: QueryRecord, result: any): number {
    if (query.queryType === "latest") {
