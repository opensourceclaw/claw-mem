...
import { BenchmarkData, FactRecord } from "./data-generator.js";
...
export class LongHorizonBenchmark extends BenchmarkCore {
  private latencies: number[] = [];
  private anchorFacts: FactRecord[] = [];
  private accuracyCurve: Map<number, number> = new Map();
  ...
  protected async runQueries(data: BenchmarkData): Promise<BenchmarkDetail[]> {
    ...
    for (const anchor of this.anchorFacts) {
      ...
      const score = this.semanticMatch(anchor.content, (firstResult?.content as string) || "");

      details.push({
        query: anchor.content.slice(0, 30),
        expected: anchor.content,
        actual: (firstResult?.content as string) || "",
        score,
      });
    }
  ...
  private queryAnchors(): number {
    ...
    for (const anchor of this.anchorFacts) {
      ...
      if (typedResults.some(r => this.semanticMatch(anchor.content, (r.content as string) || "") >= 0.5)) {
        hits++;
      }
    }
