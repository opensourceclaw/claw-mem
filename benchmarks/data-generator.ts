...
const PREFERENCE_TEMPLATES: string[] = [
...
const DISTRACTION_TEMPLATES: string[] = [
...
export class DataGenerator {
   ...
   */
  generatePreferences(count: number): { original: FactRecord[]; updated: FactRecord[] } {
  ...
  generateConversations(count: number, factsPerConversation: number): FactRecord[] {
    const conversations: FactRecord[] = [];
  ...
  generateQueries(facts: FactRecord[], count: number): QueryRecord[] {
    ...
    for (const fact of selectedFacts) {
      const queryType = this.rng.pick<QueryRecord["queryType"]>(["exact", "semantic"]);
  ...
  private generateFact(memoryType: string): FactRecord {
  ...
  private generateQueryForFact(fact: FactRecord, queryType: QueryRecord["queryType"]): string {
  ...
  private distributeTypes(total: number, types: string[]): Record<string, number> {
